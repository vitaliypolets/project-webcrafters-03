...
> Frontend — TypeScript (`.ts`, `.tsx`). Express Backend — JavaScript ES6+ (`.js`). Файли `*.types.ts` на Backend не створюємо. `validation.js` використовуємо для runtime-валідації.
...
4. Backend-код endpoint-а тримати у власному модулі (Express Backend пишемо на JavaScript):
   - `route.js`;
   - `controller.js`;
   - `service.js`;
   - `validation.ts`.
